...
export function CrewPage() {
  ...
  function addShift() {
    ...
    if (!shiftStart || !shiftEnd) {
      setAddError("Please choose both start and end time.");
      return;
    }
    if (shiftStart === shiftEnd) {
      setAddError("Start and end time can't be the same.");
      return;
    }
    if (!activeCrew) {
      setAddError("Add a crew first.");
      return;
    }

    const clash = findOverlap(shiftStart, shiftEnd, activeCrew.shifts);
    if (clash) {
      setAddError(
        `This time overlaps with an existing shift (${format12Hour(
          clash.start
    ...
    setCrews(
      crews.map((c) =>
        c.id === activeCrew.id ? { ...c, shifts: [...c.shifts, newShift] } : c
      )
    );
    setShiftStart("");
    setShiftEnd("");
        ...
        <div style={S.controlsGrid}>
          ...
          <div>
            <label style={S.fieldLabel}>
              Add shift{" "}
              {activeCrew && (
                <span style={{ color: COLOR.text }}>to {activeCrew.name}</span>
              )}
            </label>
            <div style={S.row}>
              <input
              ...
              <button
                style={{
                  ...S.secondaryBtn,
                  opacity: activeCrewId === null ? 0.5 : 1,
                  cursor: activeCrewId === null ? "not-allowed" : "pointer",
                }}
                onClick={addShift}
                type="button"
                disabled={activeCrewId === null}
              >
                + Add shift
              </button>
            </div>
            {addError && <div style={S.errorText}>{addError}</div>}
          </div>
        </div>
      </div>
